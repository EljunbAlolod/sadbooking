<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-sm font-medium text-purple-600 uppercase tracking-wider">{{ __('Marketplace') }}</p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-10">
                ...
                <div class="p-8 sm:p-10">
                    <form method="GET" action="{{ route('boarding-houses.index') }}" class="grid gap-8 lg:grid-cols-4">
                        <div class="lg:col-span-1">
                            ...
                            <x-text-input id="location" class="block w-full rounded-2xl border-slate-200 bg-slate-50/50 py-3 transition-all focus:border-purple-500 focus:ring-purple-500" type="text" name="location" :value="old('location', $filters['location'] ?? '')" placeholder="City or address..." />
                        </div>
                        ...
                        <div class="lg:col-span-1">
                            ...
                            <x-text-input id="max_rate" class="block w-full rounded-2xl border-slate-200 bg-slate-50/50 py-3 transition-all focus:border-purple-500 focus:ring-purple-500" type="number" step="0.01" name="max_rate" :value="old('max_rate', $filters['max_rate'] ?? '')" placeholder="Budget limit..." />
                        </div>

                        <div class="lg:col-span-1">
                            <label class="flex items-center gap-2 text-sm font-bold text-slate-900 mb-2">
                                <svg class="h-4 w-4 text-purple-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" /></svg>
                                {{ __('Amenities') }}
                            </label>
                            <div class="flex flex-wrap gap-2 max-h-24 overflow-y-auto pr-2 custom-scrollbar">
                                @foreach ($allAmenities as $amenity)
                                    <label class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl border border-slate-200 bg-slate-50/50 text-xs font-semibold text-slate-600 cursor-pointer hover:bg-white hover:border-purple-200 transition-all">
                                        <input type="checkbox" name="amenities[]" value="{{ $amenity->id }}" class="rounded text-purple-600 focus:ring-purple-500" @checked(in_array($amenity->id, old('amenities', $filters['amenities'] ?? []), true))>
                        ...
                        <div class="flex items-end gap-3 lg:col-span-1">
                            <button type="submit" class="flex-1 inline-flex items-center justify-center rounded-2xl bg-purple-600 px-6 py-4 text-sm font-bold text-white shadow-lg shadow-purple-200 transition-all hover:bg-purple-700 active:scale-95">
            ...
            <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                @forelse ($boardingHouses as $house)
                    <article class="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-sm hover:shadow-2xl hover:border-purple-200 transition-all duration-500">
                            ...
                            @if ($house->rooms->isNotEmpty())
                                <div class="absolute bottom-4 left-4">
                                    <div class="rounded-xl bg-white/95 backdrop-blur-md px-3 py-1.5 shadow-lg flex items-center gap-1.5">
                                        ...
                                        <span class="text-sm font-black text-purple-600">₱{{ number_format((float) $house->rooms->min('monthly_rate'), 0) }}</span>
                                    </div>
                                </div>
                            @endif
                        </div>
                        ...
                        <div class="flex flex-1 flex-col gap-4 p-8">
                            <div class="flex-1">
                                <h3 class="text-xl font-bold text-slate-900 group-hover:text-purple-600 transition-colors line-clamp-1">{{ $house->title }}</h3>
                                <p class="mt-2 flex items-start gap-2 text-sm text-slate-500">
                                    <svg class="h-5 w-5 shrink-0 text-purple-400 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z" /><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" /></svg>
                            ...
                            <div class="mt-auto pt-6">
                                <a href="{{ route('boarding-houses.show', $house) }}" class="inline-flex w-full items-center justify-center rounded-2xl bg-slate-900 px-6 py-4 text-sm font-bold text-white shadow-xl shadow-slate-100 transition-all hover:bg-purple-600 active:scale-95 group/btn">
                                    {{ __('Explore Property') }}
