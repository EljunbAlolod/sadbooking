    ...
    <div class="py-8 bg-slate-50/50 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-sm sm:rounded-3xl border border-slate-200/80 p-6 sm:p-10">
                <form method="POST" action="{{ route('landlord.boarding-houses.rooms.store', $boardingHouse) }}" enctype="multipart/form-data" 
                    x-data="{ 
                        photos: [], 
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-4">
                            <x-input-label for="photos" :value="__('Upload Room Photos')" />
                            ...
                            <!-- Preview Grid -->
                            <div x-show="photos.length > 0" class="grid grid-cols-2 gap-4 sm:grid-cols-4 mt-4">
                                <template x-for="(photo, index) in photos" :key="index">
                                    <div class="relative group aspect-square rounded-2xl overflow-hidden border border-slate-200 shadow-sm">
                                        ...
                                        <button type="button" @click="photos.splice(index, 1)" class="absolute top-2 right-2 rounded-full bg-red-500 p-1 text-white opacity-0 group-hover:opacity-100 transition-opacity">
