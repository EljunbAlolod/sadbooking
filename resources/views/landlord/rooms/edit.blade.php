                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                ...
                                <x-text-input id="room_number" name="room_number" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('room_number', $room->room_number)" required />
                                <x-input-error :messages="$errors->get('room_number')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="capacity" :value="__('Total Capacity')" />
                                <x-text-input id="capacity" name="capacity" type="number" min="1" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('capacity', $room->capacity)" required />
                                <x-input-error :messages="$errors->get('capacity')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="current_occupants" :value="__('Current Occupants')" />
                                <x-text-input id="current_occupants" name="current_occupants" type="number" min="0" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('current_occupants', $room->current_occupants)" required />
                                <x-input-error :messages="$errors->get('current_occupants')" class="mt-2" />
                            </div>

                            <div>
                                <x-input-label for="monthly_rate" :value="__('Monthly Rate (PHP)')" />
                                <x-text-input id="monthly_rate" name="monthly_rate" type="number" step="0.01" min="0" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('monthly_rate', $room->monthly_rate)" required />
                        ...
                        @if ($room->photo_path)
                            <label class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 cursor-pointer hover:bg-slate-100 transition-colors">
                                <input type="checkbox" name="remove_photo" value="1" class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
