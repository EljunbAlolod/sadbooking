        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                @forelse ($boardingHouses as $house)
                    <div class="group relative flex flex-col overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-sm hover:shadow-xl hover:border-indigo-200 transition-all duration-300">
                        <!-- Property Image -->
                        <div class="relative h-56 w-full overflow-hidden bg-slate-100">
                            @if ($house->photoUrl())
                                <img src="{{ $house->photoUrl() }}" alt="{{ $house->title }}" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-110">
                            @else
                            ...
                            <div class="absolute bottom-4 left-4 right-4 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                                <a href="{{ route('landlord.boarding-houses.show', $house) }}" class="inline-flex w-full items-center justify-center rounded-xl bg-white/95 px-4 py-2 text-sm font-bold text-slate-900 backdrop-blur-md hover:bg-white transition-colors shadow-lg">
                                    {{ __('View Property') }}
                                </a>
                            </div>
                        </div>
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            ...
                            <div class="mt-6 flex items-center gap-2 border-t border-slate-100 pt-6">
                                <a href="{{ route('landlord.boarding-houses.rooms.index', $house) }}" class="flex flex-1 items-center justify-center gap-1.5 rounded-xl bg-slate-900 px-4 py-2 text-xs font-bold text-white hover:bg-slate-800 transition-all active:scale-95 shadow-sm">
                                ...
                                <a href="{{ route('landlord.boarding-houses.edit', $house) }}" class="flex flex-1 items-center justify-center gap-1.5 rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all active:scale-95 shadow-sm">
                                    <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    {{ __('Edit') }}
                                </a>
                                <form method="POST" action="{{ route('landlord.boarding-houses.destroy', $house) }}" onsubmit="return confirm('{{ __('Are you sure you want to delete this property and all associated rooms?') }}');" class="flex shrink-0">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="flex items-center justify-center rounded-xl border border-rose-100 bg-rose-50 p-2 text-rose-600 hover:bg-rose-100 hover:text-rose-700 transition-all active:scale-90 shadow-sm" title="{{ __('Delete Property') }}">
                ...
                @empty
                    <div class="col-span-full rounded-3xl border-2 border-dashed border-slate-200 bg-white p-16 text-center shadow-sm">
                        ...
                        <p class="mt-2 text-slate-500">{{ __('Create your first property listing to start managing your rooms and tenants.') }}</p>
                        <div class="mt-8">
                            <a href="{{ route('landlord.boarding-houses.create') }}" class="inline-flex items-center rounded-2xl bg-indigo-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-indigo-100 transition-all hover:bg-indigo-700 active:scale-95">
