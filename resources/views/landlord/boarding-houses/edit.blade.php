                ...
                <form method="POST" action="{{ route('landlord.boarding-houses.update', $boardingHouse) }}" enctype="multipart/form-data" 
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="title" name="title" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('title', $boardingHouse->title)" required autofocus />
                        ...
                        <div>
                            ...
                            <x-text-input id="province" name="province" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('province', $boardingHouse->province)" required autocomplete="off" />
                            <x-input-error :messages="$errors->get('province')" class="mt-2" />
                        </div>
                        <div class="grid gap-4 sm:grid-cols-2">
                            <div>
                                <x-input-label for="city" :value="__('City / Municipality')" />
                                <x-text-input id="city" name="city" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('city', $boardingHouse->city)" required autocomplete="off" />
                                <x-input-error :messages="$errors->get('city')" class="mt-2" />
                            </div>
                            <div>
                                <x-input-label for="barangay" :value="__('Barangay')" />
                                <x-text-input id="barangay" name="barangay" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('barangay', $boardingHouse->barangay)" autocomplete="off" />
                                <x-input-error :messages="$errors->get('barangay')" class="mt-2" />
                            </div>
                        </div>
                        <div>
                            <x-input-label for="street" :value="__('Street / Landmark (optional)')" />
                            <x-text-input id="street" name="street" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" :value="old('street', $boardingHouse->street)" />
                        ...
                        @if ($boardingHouse->photo_path)
                            <label class="flex items-center gap-3 rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700 cursor-pointer hover:bg-slate-100 transition-colors">
                                <input type="checkbox" name="remove_photo" value="1" class="h-5 w-5 rounded border-slate-300 text-indigo-600 focus:ring-indigo-500" @checked(old('remove_photo'))>
                        ...
                        <div>
                            <x-input-label for="new_amenities" :value="__('Add Custom Amenities')" />
                            <textarea id="new_amenities" name="new_amenities" rows="2" class="mt-1 block w-full rounded-xl border-slate-200 shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="{{ __('Separate by comma...') }}">{{ old('new_amenities') }}</textarea>
