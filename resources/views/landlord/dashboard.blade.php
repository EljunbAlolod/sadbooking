<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <p class="text-sm font-medium text-pink-600 uppercase tracking-wider">{{ __('Boarding Hub') }}
                </p>
                <h2 class="text-3xl font-extrabold tracking-tight text-slate-900">{{ __('Dashboard') }}</h2>
                <p class="mt-2 text-sm text-slate-600">
                    {{ __('Manage your properties and track latest activity at a glance.') }}</p>
            </div>
            <div class="hidden sm:block">
                <div
                    class="inline-flex h-12 w-12 items-center justify-center rounded-2xl bg-pink-50 text-pink-600 shadow-sm">
    ...
    <div class="py-10 bg-slate-50/50 min-h-screen">
        <div class="mx-auto max-w-7xl space-y-10 px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                <div
                    class="group relative overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-8 shadow-sm hover:shadow-xl transition-all duration-300">
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">
                                {{ __('Total Boarding Houses') }}</p>
                            <p class="text-4xl font-black text-slate-900 tracking-tight">{{ $boardingHouseCount }}</p>
                        </div>
                        <div
                            class="p-4 bg-pink-50 text-pink-600 rounded-2xl group-hover:bg-pink-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                    ...
                    <div class="mt-6">
                        <a href="{{ route('landlord.boarding-houses.index') }}"
                            class="inline-flex items-center gap-1.5 text-xs font-bold text-pink-600 hover:text-pink-700 uppercase tracking-wider">
                            {{ __('Manage listings') }}
                            <svg class="h-3 w-3 transition-transform group-hover:translate-x-1" fill="none"
                ...
                <div
                    ...
                    <div class="flex items-start justify-between">
                        <div>
                            <p class="text-xs font-bold uppercase tracking-widest text-slate-400 mb-1">
                                {{ __('Pending Reservations') }}</p>
                            <p class="text-4xl font-black text-slate-900 tracking-tight">{{ $pendingReservations }}</p>
                        </div>
                        <div
                            class="p-4 bg-amber-50 text-amber-600 rounded-2xl group-hover:bg-amber-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                            <svg class="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="mt-6">
                        <a href="{{ route('landlord.reservations.index') }}"
                            class="inline-flex items-center gap-1.5 text-xs font-bold text-amber-600 hover:text-amber-700 uppercase tracking-wider">
                            {{ __('Review requests') }}
                            <svg class="h-3 w-3 transition-transform group-hover:translate-x-1" fill="none"
                                viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <section class="space-y-6">
                <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="text-xl font-bold text-slate-900">{{ __('My Boarding House') }}</h3>
                    </div>
                    <a href="{{ route('landlord.boarding-houses.create') }}"
                        class="inline-flex items-center justify-center rounded-2xl bg-pink-600 px-6 py-2.5 text-sm font-bold text-white shadow-lg shadow-pink-200 transition-all hover:bg-pink-700 active:scale-95">
                ...
                <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
                    @forelse ($boardingHouses as $house)
                        <article
                            ...
                            <div class="flex flex-1 flex-col gap-4 p-6">
                                <div>
                                    <h4
                                        class="text-lg font-bold text-slate-900 group-hover:text-pink-600 transition-colors line-clamp-1">
                                        {{ $house->title }}</h4>
                                ...
                                <div
                                    ...
                                    <span class="flex items-center gap-1">
                                        <svg class="h-4 w-4 text-pink-400" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="mt-auto flex items-center gap-3 pt-4 border-t border-slate-100">
                                    <a href="{{ route('landlord.boarding-houses.show', $house) }}"
                                        class="flex-1 inline-flex items-center justify-center rounded-xl bg-pink-600 px-4 py-2 text-xs font-bold text-white hover:bg-pink-700 transition-all active:scale-95 shadow-sm shadow-pink-100">
                                        {{ __('Details') }}
                                    </a>
                                    <a href="{{ route('landlord.boarding-houses.rooms.index', $house) }}"
                                        class="flex-1 inline-flex items-center justify-center rounded-xl border border-slate-200 bg-white px-4 py-2 text-xs font-bold text-slate-700 hover:bg-slate-50 transition-all active:scale-95">
                                        {{ __('Rooms') }}
                                    </a>
                                </div>
                            </div>
                    ...
                    @empty
                        <p class="text-sm text-slate-600 sm:col-span-3">{{ __('You have not added a boarding house yet.') }}
                        </p>
                    @endforelse
                </div>
            </section>
            ...
            <section class="overflow-hidden rounded-3xl border border-slate-200/80 bg-white shadow-xl">
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-slate-100 text-sm">
                        <thead>
                            <tr
                                class="bg-slate-50/80 text-left text-[10px] font-bold uppercase tracking-widest text-slate-500">
                        ...
                        <tbody class="divide-y divide-slate-100 bg-white">
                                ...
                                <tr class="group hover:bg-slate-50/50 transition-colors">
                                    <td class="px-8 py-5">
                                        ...
                                        <div class="text-[10px] text-slate-400 font-medium">
                                            {{ $reservation->tenant->email }}</div>
                                    </td>
                                    <td class="px-6 py-5">
                                        <div class="text-sm font-semibold text-slate-700">
                                            {{ $reservation->room->boardingHouse->title }}</div>
                                        <div class="text-[10px] font-bold text-pink-600 uppercase tracking-widest">
                                            {{ __('Room') }} {{ $reservation->room->room_number }}</div>
                                    </td>
                                    ...
                                    <td class="px-6 py-5">
                                        <span
                                            class="inline-flex rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-widest shadow-sm border {{ $statusStyles }}">
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="px-8 py-10 text-center text-slate-600">
                                        {{ __('No recent reservations.') }}</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
