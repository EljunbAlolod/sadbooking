<header class="sticky top-0 z-20 border-b border-slate-200/80 bg-white/90 backdrop-blur">
    <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <a href="{{ url('/') }}" class="flex items-center gap-2 font-semibold tracking-tight text-slate-900">
            <span class="flex h-8 w-8 items-center justify-center rounded-lg bg-pink-600 text-xs text-white">{{ strtoupper(\Illuminate\Support\Str::substr(config('app.name', 'S'), 0, 1)) }}</span>
            {{ config('app.name') }}
        </a>
        <nav class="flex items-center gap-4 text-sm font-medium">
            ...
            @else
                ...
                <a href="{{ route('register') }}" class="rounded-lg bg-pink-600 px-3 py-1.5 text-white shadow-sm hover:bg-pink-500">{{ __('Register') }}</a>
            @endauth
        </nav>
    </div>
</header>
